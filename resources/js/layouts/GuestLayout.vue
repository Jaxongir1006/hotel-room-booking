...
<template>
    ...
    >
            ...
            >
                ...
                >
                    ...
                    >
                        Home
                        <span
                            class="absolute -bottom-0.5 left-0 h-px w-0 bg-[#c9a84c] transition-all duration-300 group-hover:w-full"
                        />
                    </Link>
                    <Link
                        :href="roomsIndex().url"
                ...
                <div class="flex items-center gap-3">
                    <template v-if="user">
                        <Link
                            v-if="isAdmin"
                            :href="dashboard().url"
                            class="hidden cursor-pointer rounded-md border border-[#c9a84c]/40 px-3 py-1.5 text-xs tracking-wider text-[#c9a84c] uppercase transition-colors duration-200 hover:bg-[#c9a84c] hover:text-[#1a2744] sm:inline-block"
                        >
                            Admin
                        </Link>
                        <Link
        ...
        <main :key="page.url" class="animate-fade-in flex-1">
            <slot />
        </main>
        ...
        >
            ...
            >
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-serif text-base text-white"
                            >Aurelia Stay</span
                        >
                    </div>
                    <p class="mt-3 text-sm text-slate-400">
                        Refined hospitality, curated experiences. Every stay, a
                ...
                <div>
                    ...
                    <ul class="mt-4 space-y-2 text-sm">
                        <li v-if="!user">
                            <Link :href="login().url" class="hover:text-white"
                                >Sign in</Link
                            >
                        </li>
                        <li v-if="!user">
            ...
            >
                © {{ new Date().getFullYear() }} Aurelia Stay. All rights
                reserved.
            </div>
        </footer>
    </div>
